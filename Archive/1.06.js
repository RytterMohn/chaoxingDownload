...
(function() {
    ...
    document.onkeydown=function(e){//对整个页面监听
        ...
        if(window.location.href.includes("xueyinonline")){
            console.log("it is xueyinonline")
            var objectid=document.getElementsByClassName("ans-attach-ct")[i].getElementsByTagName('iframe')[0].getAttribute('objectid');
        }
        else if(document.getElementsByTagName("iframe")[0].contentDocument.body.getElementsByClassName("ans-attach-ct")[i]== undefined){
            var objectid=document.getElementsByClassName("ans-attach-ct")[i].getElementsByTagName('iframe')[0].getAttribute('objectid');
        }else{
...
if(protocolStr == "http:")
{   if(window.location.href.includes("xueyinonline")){
        url= 'http://mooc1.xueyinonline.com/ananas/status/' + objectid + '?flag=normal';
    }else{
...
{
    if(window.location.href.includes("xueyinonline")){
        url= 'https://mooc1.xueyinonline.com/ananas/status/' + objectid + '?flag=normal';
    }else{
        url= 'https://mooc1.chaoxing.com/ananas/status/' + objectid + '?flag=normal';
    }
}
if(window.location.href.includes("xueyinonline")){
    if(document.getElementsByClassName("ans-attach-ct")[i]==undefined){
        var FileType='.pdf';
    }else{
        var FileType=document.getElementsByClassName("ans-attach-ct")[i].getElementsByTagName('iframe')[0].getAttribute('data').type;
    }
}else{
